...
export interface SumarioLinkLike {
  nome: string;
  slug: string;
}
...
function diaFromSumarioLink(link: SumarioLinkLike): DiaLiturgico | null {
...
function findDailyLinkForDate(date: Date, links: SumarioLinkLike[]): SumarioLinkLike | null {
...
export function getReadingForDateWithLinks(
  date: Date,
  ...
  const dailyLink = findDailyLinkForDate(requestedDate, links);
...
export function getUpcomingReadingsWithLinks(
  startDate: Date,
  links: SumarioLinkLike[],
  count = 7
): DateReading[] {
  ...
  for (let index = 0; index < count; index++) {
    const reading = getReadingForDateWithLinks(cursor, links);
    if (reading) {
